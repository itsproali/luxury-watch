...
const Header = () => {
    ...
    return (
        <nav className='bg-white p-3 sticky top-0 shadow-md z-10'>
            <div onClick={() => setOpen(!open)} className='text-pink-600 w-8 cursor-pointer block md:hidden'>
                {open ? <XIcon></XIcon> : <MenuIcon></MenuIcon>}
            </div>
            <ul className={`bg-white flex flex-col sm:flex-row items-center justify-center absolute top-14 z-10 md:flex md:top-0 left-0 w-full md:sticky shadow-lg md:shadow-transparent ${open ? 'block' : 'hidden'}`}>
