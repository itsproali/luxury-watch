    ...
    <div className="mx-4 md:mx-16">
      <div className="grid grid-cols-1 md:grid-cols-2 items-center">
        ...
        <div className="home-text">
          ...
          <p className="text-1xl my-4 font-medium text-gray-500">
            Beautiful, bright AMOLED display and the broadest range of all-day
            health monitoring features keeps track of your energy levels,
      ...
      <div className="customer-reviews my-10">
        <h2 className="text-4xl font-bold text-center my-5 text-pink-500">
          Customer Reviews ({users.length})
        </h2>

        <div className="review-container grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
          {total.map((user) => (
            <DisplayUsers key={user.id} user={user}></DisplayUsers>
          ))}
        </div>

        <Link to="/reviews">
          <button className="block mx-auto my-5 font-medium px-8 py-2 bg-pink-200 text-pink-500 cursor-pointer rounded hover:bg-pink-300 hover:text-pink-700 ease-in-out duration-300">
            See All Reviews
          </button>
        </Link>
      </div>
